...
class UltralyticsROS:
    def __init__(self):
        ...
        rospy.Subscriber("/stereo/left/camera_info", CameraInfo, self.cam_info_callback)
        ...
        rospy.Subscriber("/detection/ultralytics/target_point_pub", PointStamped, self.target_processor)
    ...
    def target_processor(self, msg: PointStamped):
        if msg.header.frame_id == "None" or self.last_target_px is None or self.last_right_target_px is None:
            return
        # 检查双目相机参数相同
        if np.array_equal(self.K, self.right_K):
            stereo_K = self.K
        else:
        ...
        # target_z = STEREO_B * self.f_x / diffs
        target_z = REFER_TARGET_BOX_PIX_AREA ** 0.5 / self.target_pixel_size ** 0.5

        pixel_homogeneous = np.array([self.last_target_px[0], self.last_target_px[1], 1])
